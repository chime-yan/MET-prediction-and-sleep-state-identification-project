...
def data_statistics_2(data_df):
    data_df['time'] = pd.to_datetime(data_df['time'], format='mixed')
    data_df['time'] = data_df['time'].dt.strftime("%Y-%m-%d %H:%M:%S.%f").str[:-3]
    data_df['time'] = pd.to_datetime(data_df['time'], format='mixed')
...
if __name__ == '__main__':
    # 获取文件夹下所有 CSV 文件的路径
    file_list = glob.glob('../../result_2/*.csv')
    ...
    for i, file_path in enumerate(file_list):
        ...
        sleep_time, high_time, medium_time, small_time, static_time, overall_time = data_statistics_2(data_df)
    ...
    df_all.to_excel('../../output_and_result_files/result_2.xlsx', index=False)
    # df_all.to_excel('../../output_and_result_files/result2（测试）.xlsx', index=False)
